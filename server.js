...
const { getJobData, getJobSchema, TOTAL_JOBS, jobTitles, companies, nzLocations, industries } = require('./jobData');
...
/* NAV */
nav{background:#00529F;color:#fff;padding:0 1.5rem;display:flex;align-items:center;justify-content:space-between;height:60px;position:sticky;top:0;z-index:100}
...
/* HERO */
.hero{background:linear-gradient(135deg,#00529F 0%,#003B73 50%,#00264D 100%);color:#fff;padding:3rem 1.5rem;text-align:center}
...
.search-bar input:focus,.search-bar select:focus{border-color:#00529F}
...
.filter-chip.active,.filter-chip:hover{background:#00529F;color:#fff;border-color:#00529F}
/* LAYOUT */
...
.job-card:hover{border-color:#00529F;transform:translateY(-2px)}
...
.btn-apply{padding:.55rem 1.3rem;background:#00529F;color:#fff;border:none;border-radius:8px;font-weight:700;font-size:.85rem;cursor:pointer;transition:background .2s}
.btn-apply:hover{background:#003B73}
/* JOB DETAIL */
...
.btn-apply-big{padding:.85rem 2.5rem;background:#00529F;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:1rem;cursor:pointer;transition:background .2s}
.btn-apply-big:hover{background:#003B73}
/* PAGINATION */
...
.pagination a:hover{border-color:#00529F;color:#00529F}
.pagination .current{background:#00529F;color:#fff;border-color:#00529F}
/* BREADCRUMB */
.breadcrumb{font-size:.82rem;color:#888;margin-bottom:1rem}
.breadcrumb a{color:#00529F}
/* SITEMAP NOTE */
.info-box{background:#fff;border-radius:12px;padding:1.25rem 1.5rem;border-left:4px solid #00529F;margin-bottom:1rem;font-size:.88rem}
/* FOOTER */
...
.modal input:focus{border-color:#00529F}
.modal .btn-submit{width:100%;padding:.75rem;background:#00529F;color:#fff;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-size:.95rem}
.modal .btn-submit:hover{background:#003B73}
...
${AD_TOP}
<nav>
  <a class="brand" href="/"><span>NZ</span>Jobs<span>.co.nz</span></a>
  <div class="nav-links">
    <a href="/">Home</a>
...
<script>
  atOptions = {
    'key' : '4f24829f41a443e01bd1eae6a23156fc',
    'format' : 'iframe',
    'height' : 60,
    'width' : 468,
    'params' : {}
...
<script src="https://www.highperformanceformat.com/4f24829f41a443e01bd1eae6a23156fc/invoke.js"></script>
...
${AD_BOTTOM}
<footer>
  &copy; 2025 NZJobs.co.nz — <strong>100,000 Jobs</strong> across New Zealand |
...
<div class="job-card">
  ...
  const websiteSchema = {
    ...
    "@type": "WebSite",
    "name": "NZJobs.co.nz",
    "url": "https://nzjobs-production.up.railway.app",
    "description": "New Zealand's largest job portal with 100,000 job listings — remote and on-site across all regions",
    "potentialAction": {
      "@type": "SearchAction",
      "target": "https://nzjobs-production.up.railway.app/jobs?q={search_term_string}",
  ...
  const body = `
<div class="hero">
  <h1>Find Your Dream Job in <span class="accent">New Zealand</span></h1>
  <p>100,000 verified job listings — remote & on-site — across all 16 regions</p>
  ...
  <div class="stat-bar">
    ...
    <div class="stat"><strong>16</strong><span>Regions</span></div>
...
<div class="container">
  <div class="info-box">
    🇳🇿 New Zealand's most comprehensive job board — browse <strong>50,000 remote jobs</strong> and <strong>50,000 on-site jobs</strong> across all industries.
  </div>
   ${AD_MIDDLE}
  ...
  res.send(renderHTML({
    title: 'NZJobs.co.nz — 100,000 Jobs in New Zealand | Remote & On-site',
    meta: 'Find your next job in New Zealand. 100,000 verified listings — 50,000 remote and 50,000 on-site jobs across all 16 regions.',
    bodyContent: body,
...
<div class="hero" style="padding:1.75rem 1.5rem">
  <h1 style="font-size:1.8rem">Browse <span class="accent">100,000 Jobs</span> in New Zealand</h1>
...
<div class="container">
  ...
  res.send(renderHTML({
    title: `New Zealand Jobs — Page ${page} of ${totalPages.toLocaleString()} | NZJobs.co.nz`,
    meta: `Browse ${TOTAL_JOBS.toLocaleString()} jobs in New Zealand. Page ${page}. Remote and on-site positions across all industries.`,
    bodyContent: body,
    schema: null
  }));
});
...
app.get('/jobs/:id', (req, res) => {
  ...
  if (!id || id < 1 || id > TOTAL_JOBS) {
    return res.status(404).send(renderHTML({
      title: 'Job Not Found | NZJobs.co.nz',
      meta: 'This job listing was not found.',
      bodyContent: `<div class="container" style="text-align:center;padding:4rem 1.5rem"><h1>404 — Job Not Found</h1><p style="margin:1rem 0 2rem">This job may have been filled or removed.</p><a href="/jobs" style="color:#00529F">← Browse All Jobs</a></div>`,
      schema: null
    }));
  }
...
<div class="container">
  ...
  <div class="job-detail">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:1rem">
      ...
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:.5rem">
        ...
        <span style="font-size:.8rem;color:#888">Job ID: NZ-${String(job.id).padStart(6, '0')}</span>
      </div>
    </div>
  ...
  <div style="text-align:center;margin-top:1.5rem">
    <a href="/jobs" style="color:#00529F;font-weight:600">← Browse All 100,000 Jobs</a>
  </div>
</div>`;

  res.send(renderHTML({
    title: `${job.title} at ${job.company} — ${job.location} | NZJobs.co.nz`,
    meta: `${job.title} job at ${job.company}. ${job.isRemote ? 'Remote' : job.location}. ${job.salary}. Apply now on NZJobs.co.nz.`,
    bodyContent: body,
    schema
  }));
});
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`;
  for (let i = 1; i <= totalSitemaps; i++) {
    xml += `\n<sitemap><loc>https://nzjobs-production.up.railway.app/sitemap-${i}.xml</loc></sitemap>`;
  }
  xml += `\n</sitemapindex>`;
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`;
  for (let i = start; i <= end; i++) {
    xml += `\n<url><loc>https://nzjobs-production.up.railway.app/jobs/${i}</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>`;
  }
  xml += `\n</urlset>`;
  res.type('application/xml').send(xml);
});
...
app.get('/sitemap', (req, res) => {
  const body = `
<div class="container">
  <h1 style="margin-bottom:1rem">Sitemap — NZJobs.co.nz</h1>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;margin-top:1rem">
    <div class="job-card">
      ...
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/" style="color:#00529F">🏠 Home</a>
        <a href="/jobs" style="color:#00529F">📋 All Jobs (100,000)</a>
        <a href="/jobs?type=remote" style="color:#00529F">🌐 Remote Jobs (50,000)</a>
        <a href="/jobs?type=onsite" style="color:#00529F">🏢 On-site Jobs (50,000)</a>
      </div>
    </div>
    <div class="job-card">
      <div class="card-title">XML Sitemaps</div>
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/sitemap.xml" style="color:#00529F">📄 Sitemap Index</a>
        <a href="/sitemap-1.xml" style="color:#00529F">📄 Sitemap 1 (Jobs 1–1,000)</a>
        <a href="/sitemap-2.xml" style="color:#00529F">📄 Sitemap 2 (Jobs 1,001–2,000)</a>
    ...
    <div class="job-card">
      ...
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/jobs/1" style="color:#00529F">Job #1 (First Remote Job)</a>
        <a href="/jobs/50000" style="color:#00529F">Job #50,000 (Last Remote Job)</a>
        <a href="/jobs/50001" style="color:#00529F">Job #50,001 (First On-site Job)</a>
        <a href="/jobs/100000" style="color:#00529F">Job #100,000 (Last On-site Job)</a>
      </div>
    </div>
  </div>
</div>`;
  ...
  res.send(renderHTML({
    title: 'Sitemap | NZJobs.co.nz',
    meta: 'Complete sitemap of NZJobs.co.nz with 100,000 job listings across New Zealand.',
    bodyContent: body,
    schema: null
  }));
});
...
Allow: /
Sitemap: https://nzjobs-production.up.railway.app/sitemap.xml
Disallow: /api/`);
});
...
app.listen(PORT, () => {
  console.log(`🇳🇿 NZJobs.co.nz running on port ${PORT}`);
  console.log(`📋 ${TOTAL_JOBS.toLocaleString()} job pages ready`);
  console.log(`🏢 ${companies.length} companies hiring in New Zealand`);
  console.log(`📍 ${nzLocations.length} locations across New Zealand`);
});
